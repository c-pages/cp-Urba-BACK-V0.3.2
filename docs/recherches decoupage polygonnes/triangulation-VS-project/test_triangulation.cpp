...
static float testPointsXY[ MAX_TEST_POINTS * NUM_DIMENSIONS] ;   // to hold a set of random points
...
for (i = 0; i < g_numTestPoints; i++)  {
   ...
   ptrV->Pos.x = *ptrToPoints++;
   ptrV->Pos.y = *ptrToPoints++;
   if (NUM_DIMENSIONS == 3)  ptrV->Pos.z = *ptrToPoints++;
   else   ptrV->Pos.z = 0;
